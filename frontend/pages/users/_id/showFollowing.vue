  ...
  <div>
    <v-row>
        ...
        <section class="stats">
          ...
          <div v-if="users">
            <div class="user_avatars">
              <div v-for="followee in users" :key="followee.id">
                <NuxtLink :to="`/users/${followee.id}/show`">
                  <img
                    :src="getGravatarUrl(followee)"
                    :alt="followee.name"
                    class="gravatar"
                  />
                </NuxtLink>
      ...
      <v-col>
        <h3>Following</h3>
        <div v-if="users">
          <ul v-for="followee in users" :key="followee.id">
            <User :user="followee" :currentUser="currentUser" />
          </ul>
...
export default {
  ...
  methods: {
    async getFollowing(page) {
      return await this.$axios.$get(
        `/api/v1/users/${this.$route.params.id}/following?page=${page}`
      );
    },
    getGravatarUrl(user) {
      if (user.email) {
  ...
  watch: {
    async page() {
      const response = await this.getFollowing(this.page);
